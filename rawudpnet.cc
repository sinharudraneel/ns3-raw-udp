...
int main(int argc, char* argv[]) {
    ...
    cmd.AddValue("start04", "double: start time for n0 to n4", configs[channel04].start);
    cmd.AddValue("start40", "double: start time for n4 to n0", configs[channel40].start);
    cmd.AddValue("start05", "double: start time for n0 to n5", configs[channel05].start);
    cmd.AddValue("start50", "double: start time for n5 to n0", configs[channel50].start);
    cmd.AddValue("start14", "double: start time for n1 to n4", configs[channel14].start);
    cmd.AddValue("start41", "double: start time for n4 to n1", configs[channel41].start);
    cmd.AddValue("start15", "double: start time for n1 to n5", configs[channel15].start);
    cmd.AddValue("start51", "double: start time for n5 to n1", configs[channel51].start);
    
    cmd.AddValue("int04", "double: interval between packets for n0 to n4", configs[channel04].interval);
    cmd.AddValue("int40", "double: interval between packets for n4 to n0", configs[channel40].interval);
    cmd.AddValue("int05", "double: interval between packets for n0 to n5", configs[channel05].interval);
    cmd.AddValue("int50", "double: interval between packets for n5 to n0", configs[channel50].interval);
    cmd.AddValue("int14", "double: interval between packets for n1 to n4", configs[channel14].interval);
    cmd.AddValue("int41", "double: interval between packets for n4 to n1", configs[channel41].interval);
    cmd.AddValue("int15", "double: interval between packets for n1 to n5", configs[channel15].interval);
    cmd.AddValue("int51", "double: interval between packets for n5 to n1", configs[channel51].interval);

    cmd.AddValue("pktSize", "int: size of packets to be sent", pktSize);
    ...
    // Note that channel parameters inside each switch configuration
    // Are going to be the same, as I have set them up as ethernet switches using 
    // A CSMA channel. In order to model rates differently, separate PointToPoint connections may be required.
    // This solution seems simpler in order to achieve Ethernet II encapsulation end to end.
